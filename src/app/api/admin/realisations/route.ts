...
function buildPayload(body: Record<string, unknown>) {
  return {
    ...
    nom_ar: String(body.nom_ar ?? "").trim(),
    ...
    client_nom: body.client_nom ? String(body.client_nom).trim() : null,
    date_projet: body.date_projet ? String(body.date_projet) : null,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data, error } = await client.from("realisations").select("*").order("ordre", { ascending: true });
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { data, error } = await client.from("realisations").insert(payload).select("*").single();
...
export async function PUT(request: NextRequest) {
  ...
  try {
    const { data, error } = await client.from("realisations").update(payload).eq("id", id).select("*").single();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ realisation: data });
  } catch (e: any) { return NextResponse.json({ error: e?.message || "Erreur" }, { status: 500 }); }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    const { error } = await client.from("realisations").delete().eq("id", id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
